...
func TestRemoveKey(t *testing.T) {
	...
	for _, key := range keys {
		node, _, _, _, err := rt.FindNode(key)
		if err != nil {
			t.Fatalf("expected key %s to be found, got error: %v", key, err)
		}
		if node == nil {
			t.Errorf("expected key %s to be in a node, but got nil", key)
		}
	}
	...
	// Ensure that the key is no longer present via some lookup
...
func TestCollapseRing(t *testing.T) {
	...
	PrintHierarchyDetails(rt)

	//checkNum(rt.Size(), 5, t)
}
